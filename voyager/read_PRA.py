...
#    print(struct.unpack('<B',status_word)[0])
    ...
    #checks the status word, if its 0 then per documentation the data is bad and should be discarded
    bad=False
    if(status_word==b' 0'):
      bad=True
...
#   Can't immediately see a nicer way to do it based on the status word
      ...
      #if the status word is zero then bad is True and the values are set to zero
      if(bad==True):
        r='0'
        l='0'
